...
function ComboCustomization({ addToCart, comboDetails, isFirstPizza }) {
  ...
  const handleAddToCart = (e) => {
    ...
    // Create the combo pizza object with a default quantity of 1
    const comboPizza = {
      name: `${isFirstPizza ? 'For pizza one' : 'For pizza two'}: ${selectedSize} Combo Pizza`,
      description: `Toppings: ${[...selectedMeats, ...selectedCheeses, ...selectedFruitVegetables].join(', ')}`,
      price: totalPrice,
      quantity: 1, // Default quantity is 1
    };
  
    // Add the combo pizza to the cart
    addToCart(comboPizza);

    // Redirect to the combos page or display a success message
    // You can handle this based on your application's routing or UI flow
    // For simplicity, we won't include the navigation logic here
    setAlertMessage('Combo pizza added to cart.');
  };

  return (
    <Layout alertMessage={alertMessage} setAlertMessage={setAlertMessage}>
      <div className='header'>
        <h1>{isFirstPizza ? 'For pizza one' : 'For pizza two'} Combo Pizza</h1>
      </div>
